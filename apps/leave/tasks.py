...
@shared_task(bind=True, max_retries=3)
def send_leave_emails_async(self, leave_id):
    ...
    try:
        ...
        ).prefetch_related('cc_users').get(id=leave_id)
...
@shared_task(bind=True, max_retries=3)
def send_handover_emails_async(self, leave_id, handover_ids):
    """Send handover emails asynchronously."""
    try:
        from .models import LeaveRequest, LeaveHandover, LeaveDecisionAudit, DecisionAction
        ...
        for assignee_id, user_handovers in assignee_handovers.items():
            try:
                ...
                if LeaveDecisionAudit and DecisionAction:
                    LeaveDecisionAudit.log(
                        leave, DecisionAction.HANDOVER_EMAIL_SENT, 
                        extra={'assignee_id': assignee_id}
                    )
                
                # Create delegation reminders for each handover (2-day interval)
                for handover in user_handovers:
                    try:
                        from .models import DelegationReminder
                        ...
                        logger.error(f"Failed to create reminder for handover {handover.id}: {e}")
...
def send_leave_decision_email_async(self, leave_id):
