...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if self.target_model:
            ...
            models_to_check = [
                (model, fields) for model, fields in models_to_check 
    ...
    def _fix_model_datetime_blobs(self, model, datetime_fields):
        ...
        with connection.cursor() as cursor:
            # Get all records with potential BLOB datetime issues
            # We'll check each datetime field
            for field_name in datetime_fields:
                ...
                for i in range(0, len(blob_records), self.batch_size):
                    ...
                    batch_fixed = self._fix_datetime_batch(
                        cursor, table_name, field_name, batch
                    )
        ...
        for record_id, blob_value in batch:
            try:
                ...
                if fixed_value:
                    ...
                    if not self.dry_run:
                        # Update the record with the fixed value
                        cursor.execute(f"""
                            UPDATE {table_name} 
                            SET {field_name} = ?
                            WHERE id = ?
    ...
    def _decode_datetime_blob(self, blob_value):
        """
        Decode a datetime BLOB value into a proper ISO 8601 string.
    ...
    def _normalize_datetime_string(self, dt_string):
        ...
        This handles various datetime formats and converts them to a standard
        format that Django's SQLite converter can handle.
        """
        ...
        try:
            # First, try Django's parse_datetime
            parsed_dt = parse_datetime(dt_string)
            if parsed_dt:
                # Convert to UTC if it's aware, or make it aware in current timezone
                if timezone.is_aware(parsed_dt):
                    # Convert to UTC for storage
                    utc_dt = parsed_dt.astimezone(timezone.utc)
                else:
                    # Make it aware in current timezone, then convert to UTC
                    current_tz = timezone.get_current_timezone()
                    aware_dt = timezone.make_aware(parsed_dt, current_tz)
        ...
        try:
            ...
            # Try standard Python datetime parsing
            for fmt in [
                '%Y-%m-%d %H:%M:%S',
                '%Y-%m-%d %H:%M:%S.%f',
                '%Y-%m-%dT%H:%M:%S',
        ...
        # If all parsing fails, return the cleaned string if it looks datetime-ish
