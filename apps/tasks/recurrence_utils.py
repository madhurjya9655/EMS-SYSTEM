...
def pin_7pm_ist_on_date(d: date) -> datetime:
    """
    Return a PROJECT-TZ-aware datetime corresponding to 19:00 IST on the given date.
    """
...
# -----------------------------
# Working-day helpers
# NOTE: recurrence stepping functions below do NOT shift automatically.
# Shifting is performed by call-sites (signals/materializers) to avoid double shifts.
# -----------------------------
...
def next_working_day(d: date) -> date:
    """
    Move forward day-by-day until we hit a working day.
    Hard stop safety to avoid infinite loops.
    """
    for _ in range(0, 90):
        if is_working_day(d):
            return d
...
def preserve_first_occurrence_time(planned_dt: Optional[datetime]) -> Optional[datetime]:
    """
    Pin initial planned datetime to 19:00 IST on the same date.
    """
...
def get_next_same_time(
    ...
    *,
    ...
    """
    Step forward by mode/frequency preserving the IST wall-clock time.
...
def get_next_fixed_7pm(
    ...
    *,
    ...
    """
    Step by mode/frequency (IST date math), then pin to 19:00 IST on stepped date.
    NO working-day shift here.
    """
    m = normalize_mode(mode)
    if m not in RECURRING_MODES:
...
def get_next_planned_date(prev_planned: datetime, mode: str, frequency: int) -> Optional[datetime]:
    """
    Back-compat: same as get_next_fixed_7pm (no shift).
    """
...
def compute_next_planned_datetime(
    ...
    *,
    ...
    # force_7pm kept for signature compatibility; we always pin to 7pm in this module.
...
def is_checklist_visible_now(due_planned: datetime, now: Optional[datetime] = None) -> bool:
    ...
    if due_ist.date() > now_ist.date():
        return False
    # visible when planned time has arrived OR after 10:00 IST
    due_aware = _ensure_aware_project(due_planned)
    if due_aware is not None and due_aware.astimezone(IST) <= now_ist:
        return True
    return now_ist >= visibility_anchor_ist(due_planned)
