...
def get_next_planned_date(prev_dt: datetime, mode: str, frequency: int) -> datetime | None:
    ...
    Time is always pinned to 19:00 IST.
    """
...
def _get_handover_tasks_for_user(user, today_date: date):
    """
    Returns dict of task ids handed over TO the given user and active today.
    """
    try:
...
def dashboard_home(request):
    """
    Dashboard with handover section.

    Final visibility behavior:
    - Tasks remain visible on holidays.
    - Tasks remain visible on leave days.
    - Tasks handed over TO the user are shown.
    - Original user's own tasks are NOT hidden just because they handed them over.
    - Checklist visibility still follows 10:00 IST same-day gating.
    - Delegation and Help Ticket visibility do not get blocked by holiday/leave.
    """
    ...
    logger.info(_safe_console_text(
        f"Dashboard accessed by {request.user.username} at {now_ist.strftime('%Y-%m-%d %H:%M:%S IST')}"
    ))
    ...
    try:
        ...
        if today_only:
            ...
            if after_10:
                ...
                planned_filter = {"planned_date__lt": start_today_proj}
        ...
        all_checklists = _dedupe_by_id(base_checklists + ho_checklists)

        if today_only:
            checklist_qs = [
                c for c in all_checklists
                if (_ist_date(c.planned_date) == today_ist)
                and (getattr(c, "is_handover", False) or _should_show_checklist(c.planned_date, now_ist))
            ]
        else:
            checklist_qs = [
                c for c in all_checklists
        ...
        delegation_qs = _dedupe_by_id(base_delegations + ho_delegations)
        ...
        logger.info(_safe_console_text(
            ...
            f"| holiday_today={is_holiday_today} sunday_today={is_sunday_today} "
            f"| checklist={len(checklist_qs)} delegation={len(delegation_qs)} help={len(help_ticket_qs)} "
            f"| incoming handover: CL={len(handover_incoming['checklist'])} "
            f"DL={len(handover_incoming['delegation'])} HT={len(handover_incoming['help_ticket'])}"
        ))
    except Exception as e:
    ...
    try:
        ...
        try:
            ...
            cb_cls = Checklist.objects.filter(id__in=cb_cl_ids, status="Completed", is_skipped_due_to_leave=False)
            cb_dls = Delegation.objects.filter(id__in=cb_dl_ids, status="Completed", is_skipped_due_to_leave=False)
            cb_hts = HelpTicket.objects.filter(id__in=cb_ht_ids, status="Closed", is_skipped_due_to_leave=False)
